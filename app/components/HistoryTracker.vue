...
<template>
  <div class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-3xl p-6 shadow-sm">
    <div class="flex items-center justify-between mb-6">
      ...
      <UButton to="/dashboard/history" color="primary" variant="soft" size="sm" icon="i-lucide-arrow-right" trailing @click="playSound('nav')">View All</UButton>
    </div>
    ...
    <div class="bg-gray-50/50 dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-2xl p-5">
      <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-100 dark:border-white/5">
        <span class="font-semibold text-gray-800 dark:text-gray-200">
          Tasks for {{ new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' }) }}
        </span>
        ...
        <div v-for="habit in props.habits" :key="habit.id" class="space-y-2">
          ...
          <div class="pl-7 space-y-1">
            ...
            >
              <!-- Checkbox -->
              <button
                @click="toggleTaskOnDate(task)"
                class="shrink-0 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all"
                :style="task.completedAt && isSameDay(task.completedAt, selectedDate)
                  ? { backgroundColor: habit.color, borderColor: habit.color } 
                  : { borderColor: 'currentColor' }"
                :disabled="togglingTaskIds.includes(task.id)"
                :class="!(task.completedAt && isSameDay(task.completedAt, selectedDate)) 
                  ? 'text-gray-300 dark:text-gray-600 hover:text-gray-400' 
                  : 'text-white'"
              >
                <UIcon v-if="togglingTaskIds.includes(task.id)" name="i-lucide-loader-2" class="w-3 h-3 text-primary-500 animate-spin" />
                <UIcon v-else-if="task.completedAt && isSameDay(task.completedAt, selectedDate)" name="i-lucide-check" class="w-3 h-3" />
              </button>
