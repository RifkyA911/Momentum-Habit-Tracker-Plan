  ...
  try {
    if (wasCompleted) {
      ...
      toast.add({
        ...
        description: `Uncompleted "${task.text}" for ${dateStr}.`,
        color: 'gray'
      })
    } else {
      // Mark as completed on selected date
      await $fetch(`/api/habits/tasks/${task.id}`, {
        method: 'PATCH',
        body: {
          completed: true,
          completedAt: new Date(dateStr + 'T12:00:00') // Set midday of that date
        }
      })
      toast.add({
        title: 'Task completed',
        description: `Completed "${task.text}" for ${dateStr}.`,
        color: 'green'
      })
    }
    
    // Emit refresh to reload parent dashboard data
    emit('refresh')
  } catch (err) {
            ...
            >
              ...
              <button
                ...
                :class="!(task.completedAt && isSameDay(task.completedAt, selectedDate)) 
                  ...
                  : 'text-white'"
              >
                <UIcon v-if="task.completedAt && isSameDay(task.completedAt, selectedDate)" name="i-lucide-check" class="w-3 h-3" />
              </button>
